...
看数字        get_sfs_status
看可点按钮    list_sfs_ui
点按钮        click_sfs_ui          ← 看 delivered 判断成没成
发一次按键    press_sfs_key
...
【必须遵守的几条】
❗ 点按钮靠 click_sfs_ui；调完看返回值的 delivered 判断成没成
...
class SfsBridgePlugin(SfsUiMixin, NekoPluginBase):
    ...
    @llm_tool(
        ...
        description=(
            "点击航天模拟器的界面元素。"
            "用于操作游戏：开始游戏、载入存档、进建造、改设置、点任何按钮。"
            "推荐用法：先 list_sfs_ui 看清有哪些元素，再用 name 报元素名字点击"
            "（比记序号或坐标都可靠），也可以给 index 或 x/y。"
            "返回值的 delivered 字段说明有没有真的送达游戏，"
            "返回的 after 则是点击后的新界面清单。"
